...
async def document_websocket():
    ...
    with open("tests/pdfs/pale fire presentation.pdf", "rb") as f:
        ...
        async with session.post("http://localhost:8000/api/upload", data=form) as response:
            ...
            document_id = document["document_id"]  # Changed from id to document_id
...
@pytest.fixture
async def websocket_client(document_websocket):
    """Fixture to provide a WebSocket client"""
    session = aiohttp.ClientSession()
    async for _, document_id in document_websocket:
        # Create WebSocket client for conversation events
...
@pytest.mark.asyncio
async def test_document_upload_and_websocket(websocket_client):
    """Test 1: Basic document upload and WebSocket connection"""
